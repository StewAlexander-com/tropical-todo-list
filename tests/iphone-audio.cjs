...
(async()=>{
 for(const device of devices){
  ...
  const SupportedContext = class {};
  assert.equal(new Player(media,SupportedContext,device).Context,SupportedContext,'iPhone must retain sample-clock looping');
  const p=new Player(media,null,device);
 ...
 console.log('iPhone/iPad regressions passed: Web Audio routing, native fallback gesture playback, blocked-play retry, mute/resume, pending-play race, optional playback session');
