...
const context={self:{addEventListener:(e,f)=>handlers[e]=f,skipWaiting:async()=>{},clients:{claim:async()=>{}}},location:{origin:'https://www.troplist.com'},URL,Response,Request:class extends Request{constructor(url,opts){super(new URL(url,'https://www.troplist.com/'),opts);}},caches:{open:async()=>cache,keys:async()=>['quiet-v31','quiet-v36-release-1.2.2','other-app'],delete:async k=>deleted.push(k)},fetch:async(req,opts)=>{options=opts;if(offline)throw Error('offline');return new Response('fresh');}};
...
(async()=>{
 await life('install');assert.ok(adds.every(r=>r.cache==='reload'));assert.ok(adds.some(r=>r.url.endsWith('soundscape.js?v=1.2.2')));
 await life('activate');assert.deepEqual(deleted,['quiet-v31']);
 assert.equal(await (await request('app.js?v=1.2.2')).text(),'fresh');assert.equal(options.cache,'no-store');
